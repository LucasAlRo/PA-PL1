...
def main():
    ...
    with open(problem_name + ".pddl", 'w') as f:
        ...
        f.write("(:domain drones2-actions)\n")
        f.write("(:objects\n")
        ...
        for x in range(0, len(location_coords)):
            for y in range(0, len(location_coords)):
                if y != 0:
                    if x == 0:
                        f.write("\t(= (fly-cost-deposit depot " + location[y - 1] + ") " + str(flight_cost(location_coords, x, y)) + ")\n")
                    else:
                        f.write("\t(= (fly-cost-location " + location[x - 1] + " " + location[y - 1] + ") " + str(flight_cost(location_coords, x, y)) + ")\n")
